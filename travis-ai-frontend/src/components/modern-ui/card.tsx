  ...
  "group relative overflow-hidden rounded-xl transition-all duration-300",
  {
    variants: {
      variant: {
        ...
        outline:
          "bg-background border-1 border-primary/20 hover:border-primary/40",
        destructive: "bg-destructive text-destructive-foreground",
        dashed: "bg-background border-1 border-dashed border-border",
        elevated: "bg-background border border-border shadow-lg",
        interactive:
          "bg-background border border-border hover:scale-[1.01] hover:shadow-md",
      },
      size: {
        default: "p-6",
        sm: "p-4",
  ...
  ({ className, childClassname, variant, size, ...props }, ref) => {
    return (
      <div
        ...
        {...props}
      >
        <div className={cn("relative z-10 not-prose", childClassname)}>
          {props.children}
        </div>
      </div>
    );
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      "absolute -right-12 -top-12 h-40 w-40 rounded-full bg-primary/10 blur-3xl",
      className,
    )}
    {...props}
  />
